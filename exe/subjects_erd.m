function subjects_erd(foi, woi, min_trials, varargin)

% Parse inputs
defaults=struct('subj_dir_ext','','subj_ids',[],'freq_range', [2 35],...
    'nfreqs', 100, 'ntimesout', 800, 'plot', true, ...
    'baseline', [-500 0],'output_file','');
...
clusters(1).channels={'E30', 'E31', 'E36', 'E37', 'E41', 'E42', 'E53',...
    'E54'};
 
clusters(2).name='C4';
clusters(2).hemisphere='right';
clusters(2).region='central';
clusters(2).channels={'E79', 'E80', 'E86', 'E87', 'E93', 'E103',...
    'E104', 'E105'};
...
for cluster_idx=1:length(clusters)
    ...
    all_erds(cluster_idx).subj_erds=[];
end

if length(params.subj_ids)==0
    [included_subjects, excluded_subjects]=exclude_subjects(min_trials,...
...
for subj_idx=1:length(included_subjects)
    ...
    subj_clusters=single_subject_erd(subj_id, foi, woi,...
        'subj_dir_ext', params.subj_dir_ext, ...
        'freq_range', params.freq_range, 'nfreqs', params.nfreqs,...
        'clusters', clusters, 'ntimesout', params.ntimesout,...
        'plot', false, 'baseline', params.baseline);

    for cluster_idx=1:length(subj_clusters)
        all_erds(cluster_idx).subj_erds(subj_idx)=subj_clusters(cluster_idx).erd;
    end
end
...
if params.plot
    ...
    cluster_stderrs=[];
    for cluster_idx=1:length(clusters)
        cluster_means(end+1)=mean(all_erds(cluster_idx).subj_erds);
        cluster_stderrs(end+1)=std(all_erds(cluster_idx).subj_erds)/sqrt(length(included_subjects));        
    end
...
if length(params.output_file)>0
    ...
    title_col='subject';
    for cluster_idx=1:length(clusters)
        title_col=[title_col ',' strrep(clusters(cluster_idx).name,' ','')];
    end
    fprintf(fid, [title_col '\n']);
    for subj_idx=1:length(included_subjects)
        row=num2str(included_subjects(subj_idx));
        for cluster_idx=1:length(clusters)
            row=[row sprintf(',%1.6f',all_erds(cluster_idx).subj_erds(subj_idx))];
        end
        fprintf(fid, [row '\n']);
    end
    fclose(fid);
    ...
    fprintf(fid, 'subject,hemisphere,region,erd\n');
    for subj_idx=1:length(included_subjects)
        for cluster_idx=1:length(clusters)
            fprintf(fid, [num2str(included_subjects(subj_idx)) ',' clusters(cluster_idx).hemisphere ',' clusters(cluster_idx).region ',' sprintf('%1.6f',all_erds(cluster_idx).subj_erds(subj_idx)) '\n']);
        end
